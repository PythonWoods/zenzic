...
- **Docusaurus Full-Spec:** `DocusaurusAdapter` now parses `sidebars.ts` / `sidebars.js` statically (pure Python, no Node.js). Explicit sidebars enable precise orphan detection; autogenerated sidebars preserve the all-REACHABLE behaviour. Issue #47 closed.
