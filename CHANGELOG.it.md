...
- **`zenzic score --check-stamp`:** Verifica read-only della freshness del badge. Calcola l'URL Shields.io atteso per lo score corrente, legge `badge_stamp_files` dalla config, ed esce con codice 1 se qualsiasi file configurato contiene un URL di badge obsoleto. Mutuamente esclusivo con `--stamp`. Sostituisce il gate bash `git diff HEAD --quiet` in `just verify`, rendendo il controllo di freshness config-aware, git-agnostic e utilizzabile direttamente da qualsiasi ambiente CI. `zenzic-action` esegue `--check-stamp` automaticamente dopo `check all` (opt-out: `check-stamp: 'false'`).
...
- **`just verify` — gate freshness badge sostituito con comando nativo:** Lo step 5 ora esegue `zenzic score --check-stamp --no-header` invece della recipe bash `_badge-freshness-check`. Il gate è ora config-aware (rispetta `badge_stamp_files`) e git-agnostic.
...
- **Flag `zenzic score --no-header` aggiunto:** Nuovo flag che sopprime il banner ASCII PythonWoods. Da usare nelle pipeline CI dove `zenzic check all` ha già stampato l'header, evitando la duplicazione del banner in invocazioni sequenziali. Il ledger dello score e l'output del badge stamp non sono modificati.
- **Fix I18N di `just verify` — messaggi CI in inglese:** I messaggi `[ZENZIC FATAL]` / `[ZENZIC SUCCESS]` in `_badge-freshness-check` sono stati tradotti in inglese (confine di lingua CI: l'italiano è riservato alla documentazione utente `.md`/`.mdx`).
- **Gate freshness badge di `just verify` usa `git diff HEAD`:** Cambiato da `git diff --quiet` (working tree vs index) a `git diff HEAD --quiet` (working tree vs ultimo commit). Questo chiude un bypass via staging: `git add README.md` poteva soddisfare il controllo senza un commit effettivo. Il gate ora impone che il badge sia committato, non solo staged.
