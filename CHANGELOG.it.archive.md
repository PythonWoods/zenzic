...
Versioni storiche e log di sviluppo archiviati di Zenzic
(v0.1.0 – v0.7.1).
...
### � Patch Infrastrutturale v0.7.1

Questa release è una patch infrastrutturale silenziosa: allineamento CI/CD, correzione
matrice Nox, e enforcement di default Zero-Config. Tutti i risultati architetturali
(Anno Zero, Quartz Maturity) appartengono esclusivamente a v0.7.0. Il codebase mantiene
maturità strutturale: 1.490+ test, copertura branch >82%, e una pipeline di sicurezza blindata.
...
- **[ZRT-006] Bypass VSM: Link con Slug Assoluti Saltati Silenziosamente** (`core/validator.py`):
  ...
  2. **Lookup VSM con scope** — La soppressione Z105 `ABSOLUTE_PATH` per i prefissi di
     ...
     `_scanned_vsm_prefixes` separa i prefissi _completamente scansionati_ (quelli con ≥1
     route nella VSM) dai _plugin sorella non scansionati_ (es. `/developers/` il cui
