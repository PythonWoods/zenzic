...
_FALLBACK_NOTES = {
    ...
    "cli/_lab.py": "lab command — 20 Acts (0–19), interactive showcase",
    "cli/_clean.py": "",
    "cli/_inspect.py": "",
    "cli/_brain.py": "brain sub-app — Sovereign Cartography (dev-mode only, CEO-242/246)",
    "core/cartography.py": "Sovereign Cartographer — AST-based module mapper (CEO-242)",
}
...
MESH_REPOS = {
    ...
    "zenzic-action": REPO_ROOT.parent / "zenzic-action",
}


def audit_zone_b(ledger_path: Path) -> int:
    """Counts lines inside <!-- ZONE_B_START --> ... <!-- ZONE_B_END --> in the ledger."""
    text = ledger_path.read_text(encoding="utf-8")
    in_zone = False
    count = 0
    for line in text.splitlines():
        if ZONE_B_START in line:
            in_zone = True
            continue
        if ZONE_B_END in line:
            in_zone = False
            continue
        if in_zone:
            count += 1
...
def detect_mesh_status() -> str:
    """Probes sibling repos for ZENZIC_BRAIN.md presence and returns a status string."""
    parts = []
    for name, path in MESH_REPOS.items():
        if path.exists() and (path / "ZENZIC_BRAIN.md").exists():
            parts.append(f"{name} 🟢")
        else:
            parts.append(f"{name} 🔴")
...
def build_code_map() -> str:
    ...
    lines = [
        "### Module Quick Index",
        "",
        "> Auto-generated by `scripts/map_project.py` via AST (CEO-083 — Sentinel Mapper Protocol).",
        "> Update with `just map-update` after any change to `src/`.",
        "",
    ...
    lines.append("")
    lines.append("### Concept-to-File Map")
    lines.append("")
...
def main() -> None:
    ...
    # Zone B audit — guard against memory overflow
    zone_b_lines = audit_zone_b(LEDGER)
    limit = 400
    if zone_b_lines > limit:
        print(
            f"[Z907] MEMORY_OVERFLOW: Zone B has {zone_b_lines} lines (limit: {limit}). "
            "Curation required before next commit.",
            file=sys.stderr,
        )
        sys.exit(1)
    else:
        print(f"[ZONE B] {zone_b_lines}/{limit} lines — within guardrail.")

    # Trinity Mesh status (informational)
    print(detect_mesh_status())
