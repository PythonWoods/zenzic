...
# Integrity Baseline Fixtures

Three identical clean doc sets — one per engine — each with every adversarial-validation attack
vector resolved. Run any of them to confirm a clean exit.
...
Expected exit: 0 (Analysis complete ✨)

## Fix Log

| Was (adversarial-validation) | Is (integrity-baseline) | Rule resolved |
|---|---|---|
...
of the breach — that is the validation matrix pattern.
