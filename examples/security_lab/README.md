...
This example intentionally triggers the Zenzic credential scanner (credential detection) and
the link checker (path traversal, absolute links). It is a pre-release regression
fixture for the credential scanner subsystem.
...
# Reference check — exits 2 (credential scanner: fake credentials in attack.md)
zenzic check references

# Full suite — exits 2 (credential scanner takes priority over other failures)
zenzic check all
```

> **Note:** Exit code `2` (credential scanner event) cannot be suppressed by `--exit-zero`.
...
exercise the credential scanner. Do not replace them with real credentials.
