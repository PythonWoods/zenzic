...
### D085 — Full-Spec Alignment (Current)

**Version:** 0.7.0 · **Date:** 2026-04-26

**Issue #47 — Docusaurus Static Sidebar Parser:** `_parse_sidebars()` added to
`src/zenzic/core/adapters/_docusaurus.py`. Reads `sidebars.ts` or `sidebars.js` via
pure-Python regex (Pillar 2 — no Node.js). Autogenerated sidebar → `frozenset()`
(all REACHABLE, backwards compat). Explicit sidebar → resolves doc IDs to file paths,
enabling precise `ORPHAN_BUT_EXISTING` detection. `from_repo()` auto-detects the file.
14 new tests (SBP-01..10 + SBI-01..04). **1 246 passing.**

**Issue #52 — SARIF** closed: already implemented in D081/D082 (`_shared.py` +
`_check.py` `--format sarif`).

### Last Closed — D084 — The Obsidian Neutrality Audit

**Version:** 0.7.0 · **Date:** 2026-04-26

`docs/reference/engines.mdx` (EN + IT mirror) — 4 surgical changes:
MkDocs Route URL resolution note, Zensical Transparent Proxy elevated to :::tip,
Zensical Limitations subsection, Standalone section expanded from 17-line stub to
full section (When to use / Minimal config / Capabilities / Limitations).
README.md: HN Hook (blog link excluded until post published — `zenzic.toml` exclusion).

### Last Last Closed — D083 — The Iron Gate & Sibling Automation
