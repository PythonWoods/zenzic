...
class DocusaurusAdapter:
    ...
    def get_nav_paths(self) -> frozenset[str]:
        """Return nav-listed paths from sidebars.ts/js, or frozenset() for autogenerated.

        Calls :func:`_parse_sidebars` when a sidebar file was discovered by
        :meth:`from_repo`.  Returns an empty frozenset when:

        - No sidebar file is present (``from_repo`` was not used, or the file
          does not exist) → all files ``REACHABLE`` (current behaviour).
        - The sidebar uses ``type: 'autogenerated'`` → all files ``REACHABLE``.

        Returns:
            Frozenset of posix paths with extension relative to ``docs_root``
            (e.g. ``frozenset({'guide/install.md', 'intro.mdx'})``), or an
            empty frozenset signalling "no explicit nav".
        """
        if self._sidebar_path is not None:
            result = _parse_sidebars(self._sidebar_path, self._docs_root)
