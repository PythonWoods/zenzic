...
Historical versions and archived development logs of Zenzic
(v0.1.0 – v0.7.1).
...
## v0.7.0 Development History (Archived)

> **Note:** This section contains the granular development history of v0.7.0,
> archived for traceability. For the distilled release notes, see the
...
- **`📜 Log: v0.7.0 — Quartz Maturity`** patch-notes article published on
  the official blog (mirrors `RELEASE.md` in 30s-readable format).
- **Determinism Invariant** banner in `pyproject.toml` — formal contract
  that Zenzic ships zero AI/ML inference dependencies.
- **Zero-Legacy Policy** `forbidden_patterns` extended in
  `.zenzic.dev.toml`: `Legacy Sovereign Engine`, `structural analysis map`,
  `structural-map-check`, `Structural Memory`, `Legacy Memory` — regression-proof
  firewall preventing reintroduction of decoupled brand strings.

#### Changed — Breaking

- **`.pre-commit-hooks.yaml`** — hook id `zenzic-check-all` removed (no
  compat alias, industry-grade only) and replaced by `zenzic-verify`
  (Sentinel gate, equivalent to the Zenzic portion of `just verify`).
  Example `rev:` bumped from `v0.5.0a3` to `v0.7.0`. Entry now uses
  `--strict --quiet` (4-Gates posture). Migration for downstream
  consumers:
...
- **README.md / README.it.md** — "Zenzic Engineering Series" link on
  Dev.to replaced with sovereign canonical
  `/blog/tags/engineering-chronicles`. The blog narrative is now
  auto-canonical.

#### Removed

- `scripts/map_project.py` — superseded; no remaining callers in CI or
  `justfile`.
- `coverage.json` untracked from version control (effimero artefact;
  added to `.gitignore` to prevent re-introduction).
- Residual `Legacy Sovereign Engine` mentions in `src/zenzic/core/codes.py` and
  `src/zenzic/cli/_standalone.py` (cosmetic scrub — the CLI itself was
  already deleted in commit `41eaafc`).
...
- **Z907 `I18N_PARITY`** — new core scanner that verifies every
  base-language documentation file has a mirror in each configured
  target-language root, plus optional frontmatter-key parity (e.g.
  `title`, `description`). Language-agnostic: `base_lang` + `targets[]`
  are declared in config, no locale codes hard-coded.
- **`[i18n]` configuration section** in `zenzic.toml` (and equivalent
  `[tool.zenzic.i18n]` in `pyproject.toml`). Models: `I18nConfig`,
  `I18nSource`. Supports `extra_sources` for repositories with multiple
  documentation roots (e.g. user-facing docs + developer docs).
- **`i18n-ignore: true` frontmatter escape hatch** — files that should
  not enforce parity (drafts, lang-specific guides) opt out per-file.
- **Adaptive parallelism** — when the base-language population exceeds
  `ADAPTIVE_PARALLEL_THRESHOLD` (50 files), Z907 fan-outs the per-file
  work across a `ThreadPoolExecutor`, mirroring the rest of the scanner
  pipeline. Pure-function contract preserved (Pillar 3).
- **Hypothesis stress test** for Z907 — property-based generator covers
  deep directory nesting and Latin Extended unicode segments
  (e.g. `café/résumé`) so edge cases are caught before any
  big-bang documentation migration.

### EPOCH 4 — Unified 4-Gates Quality Pipeline (2026-05-03)

#### Changed (BREAKING for contributors)

- **`just verify` is now the atomic single entry-point** for the quality
  pipeline. Same command runs locally (via pre-push hook) and in GitHub
  Actions: locale ≡ remote, no drift.
- **`just test` reworked** to be the fast TDD inner loop: `pytest -n auto`
  with no coverage. Pillar 3 (Pure Functions) guarantees pytest-xdist
  worker isolation.
- **`just test-cov` introduced** for the audit run: serial pytest with
  coverage XML (matches CI matrix). `just verify` invokes `test-cov` so
  the `fail_under = 80` threshold is enforced before every push.
- **Nox declassed to "isolated environment manager"** — multi-version
  matrix (3.11/3.12/3.13) only. The `preflight` session removed
  (duplicated logic with `just verify`).
- **`nox -s dev` now installs both pre-commit AND pre-push hooks** so the
  Final Guard (`just verify`) runs automatically on `git push`.

#### Added

- **`pytest-xdist>=3.6`** added to the `test` dependency group.
- **`.pre-commit-config.yaml` pre-push stage** — new `just-verify` local
  hook (id `just-verify`, stages `[pre-push]`) closes the long-standing
  gap where `pre-commit install -t pre-push` installed an empty hook.
- **`.github/ISSUE_TEMPLATE/gate-bypass-postmortem.md`** — Break-Glass
  protocol template (D7). Blameless post-mortem required within 24h for
  any documented `--no-verify` bypass.
- **CONTRIBUTING.md "The 4-Gates Standard" section** documenting the
  TDD → commit → push → CI flow + Emergency / Break-Glass policy.

#### Migration

Contributors must rerun bootstrap after pulling this change:

```bash
just sync
uvx pre-commit install              # commit-stage (light hooks)
uvx pre-commit install -t pre-push  # 🛡️ Final Guard (just verify)
```

`just preflight` recipe removed; use `just verify` instead.
...
- **CHANGELOG ordering invariant (CEO-293)** codified in `Legacy Sovereign Engine` [CLOSING PROTOCOL]
...
#### CEO-DX — CLI DX Refactoring (`structural map`)
...
- **`tests/test_structural.py`:** 7 new `TestStructuralMapDX` tests (91 total in module).

#### CEO-DX-Global — Global CLI DX Standardization

- **GAP-00:** `structural_map` path argument made required (`no_args_is_help=True` now effective;
...
- **~18 new regression tests** across `test_structural.py` (`TestStructuralMapGAP00`) and `test_cli.py`
...
### D096 — Quartz Discovery, SARIF Sovereignty & Structural Curation (2026-04-30)
...
- **Zone A/B restructure** applied to all 3 public `Legacy Sovereign Engine` files (core, doc, action).
...
- **`src/zenzic/cli/_structural.py`** — `structural` sub-app (`map` command): Zone B auditor,
...
- **`just structural-map`** wired into `verify` / `preflight`. R24 Zero-Amnesia Law.
- **ADR-017** (Sovereign Cartography & Identity Gate) added to `Legacy Sovereign Engine`.
...
- **`structural analysis map --check`** — read-only audit mode. Exits 1 with `D001 MEMORY_STALE`
  if [CODE MAP] is out of sync with `src/`. Pre-commit hook `structural-map-check` added.
...
- **R27 Environmental Sovereignty** codified in `Legacy Sovereign Engine` [POLICIES].
- **`cli.mdx` EN+IT** updated: `--no-external` flag tables (zenzic-doc).

#### Tests

- **1,452 passed · ≥83% coverage** (Python 3.11 / 3.12 / 3.13). No regressions.
  `test_structural.py`: 84 tests. `test_validator.py::TestCheckExternalFlag`: 3 new tests.
...
- **`DocusaurusAdapter.get_nav_paths()` is now a true Multi-Source Aggregator.**
  ...
  _a file is REACHABLE if any user-clickable surface declares it_.
...
- **Structural Memory Architecture — `Legacy Sovereign Engine` (CEO-103-B).** Agent instructions
  migrated from `.github/copilot-instructions.md` to a root-level `Legacy Sovereign Engine` master
...
### � Infrastructure Patch v0.7.1

This release is a silent infrastructure patch: CI/CD alignment, Nox matrices correction,
and Zero-Config default enforcement. All architectural achievements (Year Zero, Quartz Maturity)
belong exclusively to v0.7.0. The codebase maintains structural maturity:
1,490+ tests, 82%+ branch coverage, and a hardened security pipeline.
...
- **[ZRT-006] VSM Bypass: Absolute Slug Links Skipped Silently** (`core/validator.py`):
  ...
  2. **Scoped VSM lookup** — The Z105 `ABSOLUTE_PATH` suppression for project-owned
     ...
     _fully-scanned_ prefixes (those with ≥1 route in the VSM) from _unscanned sibling
     plugins_ (e.g. `/developers/` whose markdown is outside the scan scope). Links
